...
const pastEvents = [
  {
    ...
    stats: [
      { icon: Users, value: "45", key: "volunteers" },
      { icon: Sprout, value: "120+", key: "plantsSaved" }
    ],
    gallery: [
  ...
  },
  {
    ...
    stats: [
      { icon: Users, value: "80", key: "volunteers" },
      { icon: Sprout, value: "200", key: "saplingsPlanted" }
    ],
    gallery: [
  ...
  },
  {
    ...
    stats: [
      { icon: Users, value: "30", key: "volunteers" },
      { icon: Sprout, value: "500 kg", key: "wasteDiverted" }
    ],
    gallery: [
                ...
                <div className="relative w-full flex flex-col rounded-[40px] bg-white border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-500 overflow-hidden group p-2">
                  ...
                  <div className="relative h-64 md:h-72 overflow-hidden rounded-[32px] bg-gray-100 shrink-0">
                    <Image
                      ...
                      alt={t(`PastEventsBlock.events.${event.id}.title`)}
                    ...
                    <div className={cn(
                      ...
                      {t(`PastEventsBlock.events.${event.id}.date`)}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col flex-1 p-6 md:p-8">
                    <Heading level={3} className="text-2xl font-display text-gray-900 mb-3 group-hover:text-primary-600 transition-colors">
                      {t(`PastEventsBlock.events.${event.id}.title`)}
                    </Heading>

                    <Text className="text-gray-600 mb-8 line-clamp-2 flex-1">
                      {t(`PastEventsBlock.events.${event.id}.shortDesc`)}
                    </Text>
                    ...
                    >
                      {t("PastEventsBlock.buttonRecap")} <ArrowRight className="w-5 h-5 ml-2 transition-transform duration-300 group-hover:translate-x-1" />
                    </Button>
                  </div>
          ...
          >
            <div className="relative h-64 sm:h-80 shrink-0">
              <Image
                ...
                alt={t(`PastEventsBlock.events.${activeEvent.id}.title`)}
            ...
            >
                    ...
                    <div key={idx} className="flex items-center gap-4 bg-gray-50 pr-6 pl-4 py-3 rounded-2xl border border-gray-100">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-medium text-gray-500">
                          {t(`PastEventsBlock.events.${activeEvent.id}.stats.${stat.key}`)}
                        </span>
                      </div>
                    </div>
                  );
              ...
              <div className="prose prose-lg prose-gray max-w-none pb-12">
                {(t(`PastEventsBlock.events.${activeEvent.id}.paragraphs`, { returnObjects: true }) as string[]).map((paragraph, idx) => (
              ...
              <Heading level={4} className="text-2xl font-display text-gray-900 mb-6 flex items-center gap-2">
                <Camera className="w-6 h-6 text-primary-500" /> {t("PastEventsBlock.galleryHeading")}
              </Heading>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-10">
                {activeEvent.gallery.map((imgUrl, idx) => (
                  <div key={idx} className="relative aspect-4/3 rounded-3xl overflow-hidden bg-gray-100 group">
                    <Image
                      src={imgUrl}
                      alt={t("PastEventsBlock.galleryImageAlt", { count: idx + 1 })}
